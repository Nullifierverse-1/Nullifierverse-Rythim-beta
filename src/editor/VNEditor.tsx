...
const VNEditor: React.FC<VNEditorProps> = ({ week, setWeek, onBack, onSave }) => {
  ...
  const handleAIGenerateScript = async () => {
    ...
    try {
          ...
          responseSchema: {
            ...
            items: {
              ...
              properties: {
                ...
                charactersPresent: { type: GenAIType.ARRAY, items: { type: GenAIType.STRING } }
              }
            }
          }
        }
      ...
      const scenes: VNScene[] = generated.map((s: any, i: number) => ({
        ...
        backgroundId: '',
        characters: s.charactersPresent.map((c: string, j: number) => ({
          ...
          <>
            ...
            <div className="w-64 border-r border-zinc-800 bg-zinc-900/30 flex flex-col">
              <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
                <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Story Scenes</h3>
                <button onClick={handleAddScene} className="p-1.5 bg-pink-600 hover:bg-pink-500 rounded-lg transition-all active:scale-90">
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
                {week.vnData?.scenes.map(scene => (
                  <div
                    ...
                    className={`w-full cursor-pointer text-left px-3 py-2.5 rounded-xl flex items-center justify-between group transition-all ${selectedSceneId === scene.id ? 'bg-pink-600/10 border border-pink-500/50 text-pink-500' : 'hover:bg-zinc-800 border border-transparent text-zinc-400'}`}
                  >
                                  ...
                                  <div className="relative group/speaker">
                                    ...
                                    <div className="absolute top-full left-0 mt-1 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl opacity-0 invisible group-focus-within/speaker:opacity-100 group-focus-within/speaker:visible transition-all z-50 p-1 min-w-[150px]">
                                      {week.vnData?.characters.map(char => (
                                        <button 
                                          key={char.id}
                                          onClick={() => handleUpdateDialogue(selectedScene.id, diag.id, { speaker: char.name })}
        ...
        {activeTab === 'ai' && (
          <div className="flex-1 p-8 overflow-y-auto custom-scrollbar">
            <div className="max-w-4xl mx-auto space-y-8 pb-20">
              {/* AI Script Writer */}
              <div className="bg-purple-600/10 border border-purple-500/30 p-8 rounded-3xl space-y-6">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-purple-600 rounded-2xl flex items-center justify-center shadow-lg shadow-purple-600/40">
                    <Sparkles className="w-6 h-6 text-white" />
                  </div>
                  <div>
                    <h2 className="text-2xl font-black uppercase tracking-tighter text-white">AI Script Writer</h2>
                    <p className="text-purple-400 text-[10px] font-black uppercase tracking-widest">Powered by Gemini 3.1 Pro</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <textarea 
                    ...
                    placeholder="Describe your story idea... (e.g., 'A tense standoff between a detective and a thief in a rainy neon-lit alleyway')"
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-6 text-lg outline-none focus:border-purple-500 transition-all min-h-[150px] resize-none shadow-inner"
                  />
                  <button 
                    onClick={handleAIGenerateScript}
                    disabled={isAIGenerating || !aiPrompt.trim()}
                    className="w-full py-4 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 rounded-2xl font-black text-sm tracking-widest uppercase transition-all active:scale-[0.98] shadow-xl shadow-purple-600/20 flex items-center justify-center gap-3"
                  >
                    {isAIGenerating ? (
                      <>
              ...
              <div className="bg-pink-600/10 border border-pink-500/30 p-8 rounded-3xl space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-black uppercase tracking-tighter text-white">AI Style Generator</h2>
                    <p className="text-pink-400 text-[10px] font-black uppercase tracking-widest">Create unique dialogue box styles</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <textarea 
                    ...
                    placeholder="Describe the style... (e.g., 'Cyberpunk neon blue with digital glitches', 'Elegant Victorian lace with gold borders')"
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-6 text-lg outline-none focus:border-pink-500 transition-all min-h-[120px] resize-none shadow-inner"
                  />
                  <button 
                    onClick={handleAIGenerateStyle}
                    disabled={isGeneratingStyle || !aiStylePrompt.trim()}
                    className="w-full py-4 bg-pink-600 hover:bg-pink-500 disabled:opacity-50 rounded-2xl font-black text-sm tracking-widest uppercase transition-all active:scale-[0.98] shadow-xl shadow-pink-600/20 flex items-center justify-center gap-3"
                  >
                    {isGeneratingStyle ? (
                      <>
                        <Loader2 className="w-5 h-5 animate-spin" />
              ...
              <div className="bg-blue-600/10 border border-blue-500/30 p-8 rounded-3xl space-y-6">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-600/40">
                    <ImageIcon className="w-6 h-6 text-white" />
                  </div>
                  <div>
                    <h2 className="text-2xl font-black uppercase tracking-tighter text-white">AI Background Generator</h2>
                    <p className="text-blue-400 text-[10px] font-black uppercase tracking-widest">Generate custom backgrounds for your scenes</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <textarea 
                    ...
                    placeholder="Describe the background... (e.g., 'A futuristic city street at night with neon lights', 'A peaceful forest clearing with sunbeams')"
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-6 text-lg outline-none focus:border-blue-500 transition-all min-h-[120px] resize-none shadow-inner"
                  />
                  <button 
                    onClick={handleAIGenerateBackground}
                    disabled={isGeneratingBackground || !aiBackgroundPrompt.trim()}
                    className="w-full py-4 bg-blue-600 hover:bg-blue-600 disabled:opacity-50 rounded-2xl font-black text-sm tracking-widest uppercase transition-all active:scale-[0.98] shadow-xl shadow-blue-600/20 flex items-center justify-center gap-3"
                  >
              ...
              <div className="bg-pink-600/10 border border-pink-500/30 p-8 rounded-3xl space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-black uppercase tracking-tighter text-white">AI Thumbnail Generator</h2>
                    <p className="text-pink-400 text-[10px] font-black uppercase tracking-widest">Generate a cover for your VN</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <textarea 
                      ...
                      placeholder="Describe the cover art... (e.g., 'A mysterious girl standing in front of a giant clock, anime style')"
                      className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-4 text-sm outline-none focus:border-pink-500 transition-all min-h-[120px] resize-none"
                    />
                    <button 
                      onClick={handleAIGenerateThumbnail}
                      disabled={isGeneratingThumbnail || !thumbnailPrompt.trim()}
                      className="w-full py-3 bg-pink-600 hover:bg-pink-500 disabled:opacity-50 rounded-xl font-black text-[10px] tracking-widest uppercase transition-all flex items-center justify-center gap-2"
                    >
                      {isGeneratingThumbnail ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                      {isGeneratingThumbnail ? 'GENERATING...' : 'GENERATE THUMBNAIL'}
                    </button>
                  </div>
                  <div className="aspect-video bg-zinc-950 rounded-2xl border border-zinc-800 flex items-center justify-center overflow-hidden relative group">
                    {week.thumbnail ? (
                      <img src={week.thumbnail} className="w-full h-full object-cover" alt="Thumbnail" />
                    ) : (
                      <div className="text-zinc-700 font-black uppercase tracking-widest text-[10px]">No Thumbnail</div>
                    )}
              ...
              {aiPreview && (
                ...
                >
                  <div className="p-6 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/50">
                    <h3 className="text-sm font-black uppercase tracking-widest text-zinc-400">AI Generated Preview</h3>
                    <div className="flex gap-2">
                      ...
                      <button 
                        ...
                        className="px-6 py-2 bg-green-600 hover:bg-green-500 rounded-xl text-[10px] font-black tracking-widest uppercase transition-all shadow-lg shadow-green-600/20"
                      >
                  ...
                  <div className="p-6 space-y-6 max-h-[500px] overflow-y-auto custom-scrollbar">
                    {aiPreview.map((scene, i) => (
                      <div key={i} className="space-y-3">
                        <div className="text-xs font-black text-purple-500 uppercase tracking-widest">Scene: {scene.name}</div>
                        <div className="space-y-2">
                          {scene.dialogue.map((diag, j) => (
                            <div key={j} className="bg-zinc-950 p-4 rounded-2xl border border-zinc-800">
                              <div className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1">{diag.speaker}</div>
                              <div className="text-sm text-zinc-300 italic">"{diag.text}"</div>
                            </div>
                          ))}
                        </div>
                      </div>
