...
const STAGES: { name: string; bpm: number; duration: number; targetNotes?: number; scrollSpeed?: number; week: number; theme: StageTheme }[] = [
  ...
  { 
    ...
    theme: {
      ...
      stage: 'rgba(255, 100, 255, 0.5)',
      particles: 'stars'
    }
  },
  { 
    name: 'Stage 2: Getting Groovy', 
    bpm: 110, 
    duration: 120,
    week: 1,
    theme: {
      ...
      stage: 'rgba(0, 200, 255, 0.5)',
      particles: 'rain'
    }
  },
  { 
    name: 'Stage 3: The Finale', 
    bpm: 120, 
    duration: 120,
    week: 1,
    theme: {
      bgTop: '#3e0000',
      bgBottom: '#ff4136',
      grid: 'rgba(255, 50, 0, 0.2)',
      stage: 'rgba(255, 100, 0, 0.5)',
      particles: 'embers'
    }
  },
  // Week 2
  { 
    name: 'Stage 1: Night Shift', 
    bpm: 130, 
    duration: 180,
    targetNotes: 600,
    week: 2,
    theme: {
      ...
      stage: 'rgba(255, 255, 255, 0.3)',
      particles: 'rain'
    }
  },
  { 
    name: 'Stage 2: Neon Streets', 
    bpm: 140, 
    duration: 180,
    targetNotes: 700,
    week: 2,
    theme: {
      ...
      stage: 'rgba(255, 0, 255, 0.4)',
      particles: 'stars'
    }
  },
  { 
    name: 'Stage 3: Cyber Overload', 
    bpm: 150, 
    duration: 180,
    targetNotes: 800,
    week: 2,
    theme: {
      ...
      stage: 'rgba(255, 0, 0, 0.5)',
      particles: 'embers'
    }
  },
  // Week 3
  { 
    name: 'Stage 1: Deep Space', 
    bpm: 160, 
    duration: 180,
    targetNotes: 800,
    week: 3,
    theme: {
      ...
      stage: 'rgba(100, 100, 255, 0.3)',
      particles: 'stars'
    }
  },
  { 
    name: 'Stage 2: Black Hole', 
    bpm: 170, 
    duration: 180,
    targetNotes: 900,
    week: 3,
    theme: {
      ...
      stage: 'rgba(255, 255, 255, 0.1)',
      particles: 'rain'
    }
  },
  { 
    name: 'Stage 3: Supernova', 
    bpm: 180, 
    duration: 180,
    targetNotes: 1000,
    week: 3,
    theme: {
      bgTop: '#330000',
      bgBottom: '#660000',
      grid: 'rgba(255, 255, 0, 0.4)',
      stage: 'rgba(255, 150, 0, 0.6)',
      particles: 'embers'
    }
  },
  // Week 4
  { 
    name: 'Stage 1: Digital Dawn', 
    bpm: 190, 
    duration: 180,
    targetNotes: 1100,
    week: 4,
    theme: {
      bgTop: '#001a1a',
      bgBottom: '#004d4d',
      grid: 'rgba(0, 255, 200, 0.2)',
      stage: 'rgba(0, 255, 150, 0.4)',
      particles: 'stars'
    }
  },
  { 
    name: 'Stage 2: Glitch Protocol', 
    bpm: 200, 
    duration: 180,
    targetNotes: 1200,
    week: 4,
    theme: {
      ...
      stage: 'rgba(200, 0, 255, 0.5)',
      particles: 'rain'
    }
  },
  { 
    name: 'Stage 3: System Purge', 
    bpm: 210, 
    duration: 200,
    targetNotes: 1400,
    week: 4,
    theme: {
      ...
      stage: 'rgba(0, 255, 0, 0.6)',
      particles: 'embers'
    }
  },
  // Bonus Stage
  { 
    name: 'EX: CHAOS THEORY (SPAMTRACK)', 
    bpm: 300, 
    duration: 180,
    targetNotes: 4000,
    scrollSpeed: 1600,
    ...
    theme: {
      bgTop: '#000000',
      bgBottom: '#330033',
      grid: 'rgba(255, 0, 255, 0.5)',
      stage: 'rgba(255, 255, 255, 0.8)',
      particles: 'stars'
    }
  },
];
...
export default function App() {
  ...
  const [selectedFreePlayStage, setSelectedFreePlayStage] = useState<number | string>(0);
  ...
  useEffect(() => {
    const loadSettings = async () => {
      ...
      if (savedBgm) {
        ...
        await bgmManager.loadTrack('menu', 'https://raw.githubusercontent.com/ShadowMario/FNF-PsychEngine/main/assets/shared/music/freakyMenu.ogg');
      }
      setIsBgmLoaded(true);
    };
    loadSettings();
  ...
  const saveAllSettings = async (newSettings: typeof settings) => {
    ...
    await saveSetting('hiddenDefaultWeeks', newSettings.hiddenDefaultWeeks);
  };
  ...
  const handleLevelComplete = (score: number, judgements: Judgements, maxCombo: number) => {
    ...
    if (customStage) {
      ...
      if (STAGES[currentStage].scrollSpeed > 2.5) difficulty = "HARD";
      else if (STAGES[currentStage].scrollSpeed < 1.5) difficulty = "EASY";
    }
  ...
  const getDynamicBg = () => {
    ...
    if (isMorning) {
      return settings.morningBg || settings.customBg || 'https://picsum.photos/seed/morning/1920/1080';
    } else {
      return settings.eveningBg || settings.customBg || 'https://picsum.photos/seed/night/1920/1080';
    }
  };
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col items-center justify-center font-sans selection:bg-pink-500/30 overflow-hidden relative">
        ...
        {screen === 'START' && (
          ...
          <div className="w-1/3 h-full flex flex-col p-12 z-10 bg-zinc-900/50 backdrop-blur-xl border-r border-white/5 relative overflow-hidden">
            ...
            <div className="mb-16 relative z-10">
              <h1 className="text-5xl font-black tracking-tighter text-white italic rainbow-text">
                NULLIFIERVERSE
              </h1>
            ...
            <div className="flex flex-col gap-3 flex-1">
              {[
                ...
                { id: 'ARCHIVE', label: 'ARCHIVE', icon: <Archive className="w-6 h-6" />, screen: 'ARCHIVE', color: 'from-emerald-500 to-teal-600' },
          ...
          <div className="relative z-10 w-full h-full flex flex-col p-10 pb-16">
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              <div className="w-1/2 flex flex-col gap-6 overflow-y-auto custom-scrollbar pr-4 pb-10">
                {[
                  { id: 1, title: 'Week 1 (the beginning)', color: 'bg-pink-500', hover: 'group-hover:bg-pink-400' },
                  { id: 2, title: 'Week 2 (neon nights)', color: 'bg-cyan-500', hover: 'group-hover:bg-cyan-400' },
                  { id: 3, title: 'Week 3 (cosmic chaos)', color: 'bg-purple-500', hover: 'group-hover:bg-purple-400' },
                  { id: 4, title: 'Week 4', color: 'bg-emerald-500', hover: 'group-hover:bg-emerald-400' },
                ].filter(w => !settings.hiddenDefaultWeeks.includes(w.id)).map((week) => (
                  <div key={week.id} className="relative group w-full max-w-lg shrink-0 flex items-center">
                    ...
                    >
                      ...
                      <div className={`absolute inset-0 flex items-center justify-between px-12 ${week.color} ${selectedWeek === week.id ? 'brightness-110' : week.hover}`} style={{ clipPath: 'polygon(8% 0, 100% 0, 92% 100%, 0% 100%)' }}>
                        <span className="text-white text-3xl font-['Anton'] italic tracking-wider drop-shadow-[2px_2px_0_rgba(0,0,0,0.5)]">
                          Week {week.id} <span className="font-sans text-xl opacity-80">{week.title.replace(`Week ${week.id} `, '')}</span>
                        </span>
                    ...
                    <button 
                      onClick={(e) => { e.stopPropagation(); setItemToDelete({ type: 'defaultWeek', id: week.id }); }}
                    ...
                    {typeof selectedWeek === 'number' ? (
                      STAGES.filter(s => s.week === selectedWeek).map((stage, idx) => (
                        <span key={idx} className="text-white text-2xl italic font-sans tracking-widest drop-shadow-[2px_2px_0_rgba(0,0,0,1)] opacity-90">
                          {idx + 1}. {stage.name.split(': ')[1] || stage.name}
                        </span>
                      ))
                    ) : (
                      allCustomWeeks.find(w => w.id === selectedWeek)?.tracks.map((trackId, idx) => {
                        const stage = allCustomStages.find(s => s.id === trackId);
                        return (
                          <span key={idx} className="text-white text-2xl italic font-sans tracking-widest drop-shadow-[2px_2px_0_rgba(0,0,0,1)] opacity-90">
                            {idx + 1}. {stage ? (stage.name.split(': ')[1] || stage.name) : 'Unknown Track'}
                          </span>
                        );
                      })
                    )}
                    {(typeof selectedWeek === 'number' ? STAGES.filter(s => s.week === selectedWeek).length : allCustomWeeks.find(w => w.id === selectedWeek)?.tracks.length) === 0 && (
                ...
                <div className="w-full max-w-lg aspect-video bg-white/10 backdrop-blur-md border-4 border-white/20 rounded-xl relative overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] flex items-center justify-center mt-4">
                  ...
                  <span className="text-white text-4xl font-['Playfair_Display'] italic drop-shadow-[3px_3px_0_rgba(0,0,0,1)] z-10">
                    {typeof selectedWeek === 'number' 
                      ...
                      : allCustomWeeks.find(w => w.id === selectedWeek)?.name || 'Custom Week'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 flex justify-between items-end w-full px-4">
              ...
              <button
                ...
                className="relative group h-20 w-80 transition-all duration-200 hover:scale-105 mr-8"
              >
                {/* Drop shadow */}
          ...
          <div className="relative z-10 w-full h-full flex flex-col p-10 pb-16">
                ...
                <div className="w-full max-w-lg aspect-video bg-white/10 backdrop-blur-md border-4 border-white/20 rounded-xl relative overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] flex items-center justify-center mt-4">
                  ...
                  <span className="text-white text-4xl font-['Playfair_Display'] italic drop-shadow-[3px_3px_0_rgba(0,0,0,1)] z-10 text-center px-4">
                    {typeof selectedFreePlayStage === 'number' 
                      ? STAGES[selectedFreePlayStage]?.name.split(': ')[1] || STAGES[selectedFreePlayStage]?.name
            ...
            <div className="mt-8 flex justify-between items-end w-full px-4">
              ...
              <motion.button
                ...
                className="relative group h-20 w-80 transition-all duration-200 mr-8"
              >
                {/* Drop shadow */}
                <div className="absolute inset-0 bg-black translate-y-3 translate-x-3" style={{ clipPath: 'polygon(15% 0, 100% 0, 85% 100%, 0% 100%)' }} />
                {/* Orange border */}
                ...
                >
                  ...
                  <div className="mt-16 space-y-6">
                    ...
                    {activeSettingsCategory === 'Reset Data' && (
                      <div className="flex flex-col items-center justify-center h-full space-y-8 py-12">
                        <div className="text-center">
                          <h4 className="text-3xl font-black italic text-white uppercase mb-2">Reset Data</h4>
                          <p className="text-blue-300 font-bold uppercase tracking-tighter">Restore hidden default weeks and clear preferences</p>
                        </div>
                        <button
                          onClick={() => saveAllSettings({ ...settings, hiddenDefaultWeeks: [] })}
      ...
      {screen === 'PLAYING' && (
        ...
        >
          {!isFullscreen && (
            <div className="w-full flex justify-between items-center px-4 py-2 bg-zinc-900 rounded-xl border border-zinc-800">
              <span className="font-bold text-zinc-400">
                {(gameMode === 'EDITOR' || gameMode === 'WEEK_PLAYTEST') ? customStage?.name : STAGES[currentStage].name}
              </span>
          ...
          <RhythmGame 
            ref={gameRef}
            bpm={(gameMode === 'EDITOR' || gameMode === 'WEEK_PLAYTEST' || gameMode === 'CUSTOM_STORY' || gameMode === 'FREEPLAY') && customStage ? customStage.chart.bpm : STAGES[currentStage].bpm}
            duration={(gameMode === 'EDITOR' || gameMode === 'WEEK_PLAYTEST' || gameMode === 'CUSTOM_STORY' || gameMode === 'FREEPLAY') && customStage ? 9999 : STAGES[currentStage].duration}
            targetNotes={(gameMode === 'EDITOR' || gameMode === 'WEEK_PLAYTEST' || gameMode === 'CUSTOM_STORY' || gameMode === 'FREEPLAY') && customStage ? 0 : STAGES[currentStage].targetNotes}
            scrollSpeed={(gameMode === 'EDITOR' || gameMode === 'WEEK_PLAYTEST' || gameMode === 'CUSTOM_STORY' || gameMode === 'FREEPLAY') && customStage ? customStage.chart.scrollSpeed : STAGES[currentStage].scrollSpeed}
      ...
      {screen === 'NEXT_SONG_RESULTS' && results && (
        <ResultsScreen
          type="SONG_CLEARED"
          results={results}
          songName={gameMode === 'STORY' ? STAGES[currentStage].name : (customStage?.name || 'Unknown')}
      ...
      {screen === 'LEVEL_COMPLETE' && results && (
        <ResultsScreen
          ...
          nextText={gameMode === 'EDITOR' ? "BACK TO EDITOR" : gameMode === 'FREE_PLAY' ? "BACK TO FREEPLAY" : "FINISH WEEK"}
      ...
      {screen === 'WEEK_RESULTS' && playtestWeek && (
        <ResultsScreen
          ...
          results={{
            ...
            maxCombo: Math.max(...weekPlaytestResults.map(r => r.maxCombo), 0)
          }}
          clearTime={results?.clearTime || "00:00"}
          accuracy={(() => {
